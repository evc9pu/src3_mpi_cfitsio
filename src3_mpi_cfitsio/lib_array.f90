! MD5 of template: 152734673bb6fd814eb75a44dda4b495
...
module lib_array
  ...
  interface index_array_1d
     module procedure index_array_1d_sp
     module procedure index_array_1d_dp
  end interface
...
contains
  ...
  subroutine sort_dp(x,y)
    ! Sort one or two arrays. If two arrays are provided, the second array
    ! is sorted in the same order as the first
    implicit none
    real(dp),dimension(:) :: x
  ...
  subroutine smooth_2d(array,sigma)
    ...
    integer :: nx,ny
    real,pointer :: array_orig(:,:),array_count(:,:)
