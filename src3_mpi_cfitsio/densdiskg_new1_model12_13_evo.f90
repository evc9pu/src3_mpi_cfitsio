subroutine densdiskg_new(rmincgs,iter,lstar,lacc,lacc2)
  ...
  real(8) :: rad,dr,dr3,alpha,rho,cyn_c,sint
  ...
  real(8) :: rho2,v_Kc,alphal,alphah,alpha_temp,gexpmax
  real(8) :: massc_in,rmincgs_in,mdotcgs_in,rmaxd_in,rinfall_in,fd_in
  real(8) :: mdotcorecgs_in,dt,diffsigma,sumsigma,tnow,gexp1,thet_temp

  real(8) :: msevo,rsevo
  real(8) :: mcevo,msdotevo,pwevo,epsilonevo,thetaevo,fescevo
  
  real(8) :: sigmaarr(nrg-1),zmrarr(nrg-1),sigmadotarr(nrg-1)
  real(8) :: sigmaarr1(nrg-1),sigmadotarr_new(nrg-1)

  integer :: ir_zdmax,iiter,ievo,iievo
...
10      massc_in=massc
        ...
        close(1)
        close(2)
        open(1,file=trim(par_dir)//'/'//trim(histdir)//'/acchist.dat')
        open(2,file=trim(par_dir)//'/'//trim(histdir)//'/history.dat')
        do iievo=1,1000
           read(1,*) ievo,msevo,mcevo,msdotevo,pwevo,epsilonevo,thetaevo,fescevo
           read(2,*) ievo,msevo,rsevo
           if (msevo.gt.massc) then
              exit
           end if
        end do
        close(1)
        massc_in=msevo
        rmincgs_in=rsevo*rsol
        rmincgs_in=rmincgs
        mdotcgs_in=msdotevo*msol/365.25d0/3600.d0/24.d0
        rmaxd_in=0.684*betac*(mcevo/msevo/(1.d0+fd))*(mcevo/mcore)**(2.d0/3.d0)*rcore*autors
        if (ifprint) then
           print*,'rmaxd (au) change',rmaxd/autors,rmaxd_in/autors
           print*,'massc change',massc,massc_in
           print*,'thet change',thet1,thetaevo
           print*,'mdot change',mdotcgs*365.25d0*3600.d0*24.d0/msol,msdotevo
        end if
        rinfall_in=rmaxd_in*(sin(thetaevo*deg2rad))**2
        fd_in=fd
        mdotcorecgs_in=(1.d0+fd+fw*fwesc)*mdotcgs_in/cos(thetaevo*deg2rad)
        ...
        dt=(msevo-massc)*msol/(mdotcgs+mdotcgs_in)*2.d0 ! in sec
        sigmadotarr_new=(sigmaarr1-sigmaarr)/dt
        
!!$        if (ifprint) then
!!$           do ir=1,nrg-1
!!$              print*,ravearr(ir)/autors,sigmadotarr_new(ir)
!!$           end do
!!$        end if
...
!           gexp=(zp/zmr)**2
           ...
           if (gexp.gt.gexpmax) dens=0.d0
           zp=rad*abs(cost)
           rp=rad*abs(sint)
           cyn_c=1.d0+14.d0*log(1.d0+0.07d0*zp)
           if (rp.le.cyn_c) dens=0.d0
           do ip=1,npg-1
              temp=tdave(ir,it,ip)
...
!              rhod(ir)=rhod(ir-1)
!              cyndisk(ir)=cyndisk(ir-1)
              thet_temp=pi/2.d0-sqrt(gexp1)*hdisk(ir)/rad
              rhod(ir)=midrho(ir)*exp(-0.5d0*gexp1)
              zdisk(ir)=rad*cos(thet_temp)
              cyndisk(ir)=rad*sin(thet_temp)
           end if
        end do
