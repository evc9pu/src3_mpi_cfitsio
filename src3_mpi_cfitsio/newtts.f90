...
!     if (np.lt.nporig/10) np=nporig/100
     if (np.gt.nporig) np=nporig
     if (np.lt.npmin) np = npmin

     if (ifprint) print*,'newtts_lucy, np per iteration, np_final',np,nporig
  ...
  do while (converge.eq.'no')
     ...
     call wrimsg('TTS',cmsgnm)
     if (ifprint) print*,''
     if (ifprint) print*,'Starting Lucy iteration ',iter,' at CPU time ',cpusec
     if (ifprint) print*,''
     ...
     if (ifprint) print*,'done with disk'
     ...
     call wrimsg('TTS',cmsgnm)
     if (ifprint) print*,''
     if (ifprint) print*,'Elapsed time for Lucy iteration ',iter,': ',&
          cpusec-cpusec_old,' (sec CPU time)'
     if (ifprint) print*,''
...
!        print*,'at user-specified max iteration--CAUTION, T may not have converged!'
        converge='yes'
      end if

      call initarr(.false.,iter+1)

  end do

  if (ifprint) print*,'total number of iterations ',iter
  if (ifprint) print*,'Initial iterations for temperature done!'
...
0990 call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  ...
  WRITE(12,'(a)') 'Initial iterations for temperature done!'
  close(12)
